...
namespace WindowsGame1
{
    public class Layer
    {
        ...
        public void LoadContent(Map map, string layerID)
        {
            ...
            for (int i = 0; i <attributes.Count; i++)
            {
                for (int j = 0; j < attributes<em>.Count; j++)
                {
                    ...
                    {
                        case "TileSet":
                            tileSheet = content.Load<Texture2D>("TileSets/" + contents<em>[j]);
                            break;
                        case "Solid":
                            solid.Add(contents<em>[j]);
                            break;
                        case "Motion":
                            motion.Add(contents<em>[j]);
                            break;
                        case "StartLayer":
                            ...
                            for (int k = 0; k <contents<em>.Count; k++)
                            {
                                string[] split = contents<em>[k].Split(',');
                                tempTiles.Add(new Tile());
 
                                if (solid.Contains(contents<em>[k]))
        ...
        public void Update(GameTime gameTime)
        {
            ...
            {
                for (int j = 0; j < tiles<em>.Count; j++)
                {
                    tiles<em>[j].Update(gameTime);
                }
            }
        }
