    ...
    public class Layers
    {
        ...
        public void LoadContent(ContentManager content, string mapID)
        {
            ...
            for (int i = 0; i <attributes.Count; i++)
            {
                for (int j = 0; j < attributes<em>.Count; j++)
                {
                    ...
                    {
                        case "TileSet":
                            tileSet = this.content.Load<Texture2D>("TileSets/" + contents<em>[j]);
                            break;
                        case "TileDimensions":
                            string[] split = contents<em>[j].Split(',');
                            tileDimensions = new Vector2(int.Parse(split[0]), int.Parse(split[1]));
                            break;
                        case "StartLayer":
                            for (int k = 0; k <contents<em>.Count; k++)
                            {
                                split = contents<em>[k].Split(',');
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            for (int k = 0; k <tileMap.Count; k++)
            {
                ...
                {
                    for (int j = 0; j < tileMap[k]<em>.Count; j++)
                    {
                        spriteBatch.Draw(tileSet, new Vector2(j * tileDimensions.X, i * tileDimensions.Y),
                            new Rectangle((int)tileMap[k]<em>[j].X * (int)tileDimensions.X,
                                (int)tileMap[k]<em>[j].Y * (int)tileDimensions.Y,
